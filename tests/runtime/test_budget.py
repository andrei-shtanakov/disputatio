...
def test_schema_retry_limit_is_not_reset_by_the_budget_charge(
    ...
    кончается) и отсутствием пересадки FSM внутри петли. Начисление и
    пересадка разведены (SPEC-001 §4.1): шаг, закрывший сессию в `FAILED`,
    расход своих попыток начисляет (`budget.charge_failed_step`), но FSM
    при этом не пересаживает — счётчику повторов сбрасываться не на чем.
    """
    harness = _make_harness(
        tmp_path,
        monkeypatch,
