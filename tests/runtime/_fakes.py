...
`GitOps` — `runtime_checkable`-протокол, и его расширение шестью операциями
adoption/reconciliation (SPEC-002 §3.1, §7.3, §8.1) обязано было уронить
каждый фейк порта разом: `isinstance` смотрит на наличие атрибутов, а
pyrefly — ещё и на совместимость сигнатур в `RuntimeDeps(git=…)`. Дописывать
шесть заглушек в двадцать с лишним фейков значило бы завести двадцать копий
одного решения, расходящихся при следующем расширении порта; поэтому
заглушки живут здесь, а фейлы наследуют их.

**Любая из шести операций — провал теста, а не тихий ответ.** Ни один шаг
