...
* **Диспетчеризация по фазе.** Сессия, поднятая в `VERIFYING` раунда 2,
  ...
  Пинится ОТФИЛЬТРОВАННЫМ логом обращений к портам (`_port_calls`), а не
  набором «вызвано/не вызвано»: «PROPOSING переигран» — это лишняя метка в
  начале лога, и увидеть её можно только по порядку. Сырой общий лог для
  этого негоден: в него пишет ещё и sink, а `gate_started` эмитится ДО
  прогона гейтов ([DESIGN-004], залочено в `test_step_verifying.py`), то
  есть законно занимает в нём нулевую позицию. Утверждать здесь
  `log[0] == "verifier.verify"` значило бы пинить чужой инвариант с
  обратным знаком.
...
def test_resume_runs_the_saved_phase_and_does_not_rerun_proposing(
    ...
    Первое обращение к порту — прогон гейтов, а не вызов автора: `PROPOSING`
    раунда 2 уже состоялся, и его повтор переписал бы работу автора, выдав
    за раунд 2 пустое дерево. Порядок читается по `_port_calls` — сырой лог
    сюда не годится: `gate_started` уходит в него ДО прогона гейтов, и
    нулевая позиция принадлежит ему по контракту [DESIGN-004]. Ревьюер
    обрывается сразу после перехода — дальше цикла тест не ведёт, ему
    хватает начала.
    """
