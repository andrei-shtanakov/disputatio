...
def test_snapshot_without_gates_and_attachments_reads_as_empty_not_as_error() -> None:
    ...
    Сессия без гейтов законна ([REQ-010]), и загрузчик её не судит:
    требовать хотя бы один гейт значило бы завести здесь второе мнение о
    §5. Судит §4.3 — пустой набор даёт `indeterminate`, то есть вне
    `analyze` такая сессия не сойдётся и уйдёт в `ESCALATED` с частичным
    экспортом. Это исход прогона, а не отказ разбора. Задача без вложений
    законна тем более.
    """
