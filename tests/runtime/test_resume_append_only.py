...
# хука, — а `raise` безусловен: сессия переводится в `FAILED`, отказ сверки
# и I3 уходят наружу как есть, прочий сбой политики — `LifecyclePolicyFailed`
# с исходным в `__cause__` (#113). I3 обёртке не подлежит: обёрнутый, он стал
# бы членом `SESSION_CLOSING_ERRORS` и драйвер его проглотил бы. Ловля именно
# `Exception`, а не `BaseException` и не `finally`: kill во время хода автора
# обязан оставлять сессию в `PROPOSING`, то есть переигрываемой, а не
# помечать её отказавшей.
#
