...
#: Границы sweep'а, на которых внешний эффект БЕЗ durable-guard'а законно
#: исполняется второй раз: запись, следующая сразу за самим эффектом. Для
#: `git reset` возврата это commit point (§7.3 шаг 4), для экспорта — запись
#: перехода `EXPORTING → DONE`. Числа замерены, а не выведены: единственный
#: способ узнать номер границы — прогнать все и посмотреть.
RESET_REPLAY_BOUNDARY: Final = 8
EXPORT_REPLAY_BOUNDARY: Final = 15


def replayed_once(base: int, crash_on_save: int, replay_at: int) -> int:
    """Сколько раз эффект исполнится, если оборваться на границе `crash_on_save`.

    Ровно `base + 1` на своей границе и ровно `base` на всех остальных.
    Точное число, а не «не больше» — неравенство пропустило бы новый повтор
    молча, ради чего оракул и заводился.
    """
    return base + 1 if crash_on_save == replay_at else base
...
def test_every_manifest_write_is_a_replayable_boundary(
    ...
    **Чего этот sweep НЕ делает: он не закрывает дыру в покрытии.** Проверено
    ...
    перенести reset из commit point'а в запись интента; добавить партиальный
    экспорт после каждой сессии) ловят и точечные тесты — `crash_10_parked`,
    `crash_4`/`crash_5`, `happy_path`/`crash_7` соответственно. Ценность
    sweep'а не в эксклюзивной находке, а в единообразии: те же три инварианта
    применяются на КАЖДОЙ границе, поэтому повтор эффекта на границе, для
    которой точечного теста никто не написал, будет пойман. Это страховка
    против будущей формы кода, а не против сегодняшней.
    ...
    **Правило, по которому эффект переигрывается или нет, одно.** Runner
    ...
    `manifest.json` как commit marker). Поэтому каждый из них законно
    исполняется дважды — ровно на одной границе, той, что идёт сразу после
    него.

    Ожидания точные, а не «не больше двух»: замер даёт `resets=2` только на
    границе 8 и `export=2` только на границе 15, всюду иначе — по одному.
    Точное равенство ловит появление ЛЮБОГО нового повтора, тогда как
    неравенство пропустило бы его молча.

    **Про `test_crash_3_*` и `test_crash_10_*`.** Законность повторного
    вызова драйвера определяется не местом инжекции обрыва, а тем,
    приземлился ли durable-эффект драйвера: в `crash_3` (падение ДО записи)
    не приземлился, и повтор обязателен — тест его и требует; в обоих
    `crash_10` (падение ПОСЛЕ записи) приземлился, и повтор запрещён — тесты
    требуют обратного. Границы этого sweep'а все лежат после возврата
    драйвера, то есть после приземления его эффекта, — потому здесь и стоит
    равенство.
    """
    ...
    ), "reset переигрывается ровно на границе сразу после себя, и больше нигде"
