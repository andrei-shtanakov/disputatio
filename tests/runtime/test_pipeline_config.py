...
def test_unknown_pipeline_keys_fail_closed_at_every_schema_level(
    ...
    конкретный чеклист, его `items` и запись `pipeline.gates` — каждый из
    этих пяти уровней обязан отвергать чужой ключ сам, а не полагаться на
    то, что его отловит соседний уровень.
    """
    monkeypatch.setenv("XDG_STATE_HOME", str(tmp_path / "state-home"))
