...
Файл отдельный не по вкусу, а по правилу: `test_cli_run.py` был залочен
red-чекпоинтом задачи побайтово, и дыры, найденные пробой уже после него,
закрывались рядом, а не правкой залоченного набора. **Лок снят вместе с
локальным TDD-гейтом** (снос 2026-08-22, PR #40): байт-лок исполнял тот
гейт, а `spec/.tdd-evidence/` с тех пор — архив сертификации, которому
никто не подчиняется. Разделение остаётся как история появления дыр, а не
как запрет править соседа.
