...
`test_cli_run_wiring.py` были залочены red-чекпоинтами своих задач
побайтово, а дыра здесь появилась вместе с начислением бюджета — то есть
уже после них. **Лок снят вместе с локальным TDD-гейтом** (снос
2026-08-22, PR #40), который его и исполнял; разделение осталось как
история, не как запрет.
...
def _profile() -> RuntimeConfig:
    ...
    return RuntimeConfig(
        ...
        # Гейт настоящий и пустой: без выполненного гейта `develop`-сессия
        # не сходится (§4.3) и до предмета теста — исхода после начисления
        # бюджета — дошла бы не тем путём.
