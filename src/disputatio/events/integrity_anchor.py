...
class IntegrityAnchor:
    ...
    def _append(self, record: AnchorRecord) -> None:
        ...
        эксклюзивной блокировкой (`events.file_lock`). Писатель у журнала
        один (SPEC-002 P11): его единственность обеспечивает операторская
        дисциплина, и блокировка — дополнительная защита на случай её
        нарушения, а не поддержка конкурентной записи. Сужает она ровно
        этот ущерб: усечение режет файл по смещению, посчитанному ДО чужой
        записи, и два `disp pipeline resume` над журналом с оборванным
        хвостом, запущенные вопреки модели, сняли бы один и тот же снимок —
        второй дописал бы полную `pre_turn`, первый срезал бы её как
        «хвост», и ход остался бы без снапшота P9.
        """
        with exclusive_lock(self._path):
