...
def _changed_lines(patch: str) -> set[str]:
    ...
    Классификация строки `+`/`-` как изменения зависит не от самой строки,
    а от СОСТОЯНИЯ открытого ханка — булева флага, который выставляет
    заголовок `@@` и сбрасывает заголовок следующего файла `diff --git `
    (FR-14). Строки `+`/`-` до первого `@@` в это состояние не засчитываются
    (BEH-01); после `diff --git ` состояние снова закрыто, и метаданные до
    следующего `@@` изменениями не считаются (BEH-02). Заголовки
    `+++ `/`--- ` всегда предшествуют `@@` того же файла, то есть встречаются
    при закрытом состоянии, поэтому уже отсечены — отдельного исключения для
    них не нужно, и оно бы ошибочно резало добавленную строку содержимого
    вида `++ tail` (полная строка патча `+++ tail`, BEH-05). Нормализация —
    strip маркера и хвостовых пробелов.
    """
    changed: set[str] = set()
    in_hunk = False
