...
def _changed_lines(patch: str) -> set[str]:
    ...
    Строки `+`/`-`; заголовки `+++ `/`--- ` всегда предшествуют `@@` того же
    файла, поэтому уже отсечены проверкой открытого ханка ниже — отдельного
    исключения для них не нужно, и оно бы ошибочно резало добавленную
    строку содержимого вида `++ tail` (полная строка патча `+++ tail`,
    BEH-05). Нормализация — strip маркера и хвостовых пробелов. Строки
    `+`/`-` до первого заголовка ханка `@@` не учитываются (BEH-01).
    Заголовок следующего файла (`diff --git `) закрывает открытый ханк —
    метаданные между ним и следующим `@@` не считаются изменениями (BEH-02).
    """
    changed: set[str] = set()
    in_hunk = False
