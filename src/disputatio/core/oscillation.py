...
def _changed_lines(patch: str) -> set[str]:
    ...
    Строки `+`/`-`, исключая заголовки `+++ `/`--- ` (git всегда добавляет
    ...
    первого заголовка ханка `@@` не учитываются (BEH-01). Заголовок
    следующего файла (`diff --git `) закрывает открытый ханк — метаданные
    между ним и следующим `@@` не считаются изменениями (BEH-02).
    """
    changed: set[str] = set()
    in_hunk = False
