...
возвращается LF-нормализованным (байт-в-байт для LF-входа). Структурные ошибки (нет фронтматтера,
...
def parse_proposal(text: str) -> tuple[ProposalFrontmatter, str]:
    ...
    Вход нормализуется до разбора: ``\\r\\n`` → ``\\n``, ведущие пустые
    строки отбрасываются (агентский вывод не byte-stable, ADR-008).
    Нормализованный документ обязан начинаться с ``---\\n``; блок до
    следующей строки, равной ``---``, читается ``yaml.safe_load``; остаток
    возвращается как LF-нормализованное тело — байт-в-байт для LF-входа.
    Строки ``---`` внутри тела границей не считаются.
    """
    text = text.replace("\r\n", "\n").lstrip("\n")
