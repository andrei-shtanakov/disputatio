...
class VerifierRunner:
    ...
    def __init__(
        ...
        *,
        ...
        бы «пропущенным» с чужой причиной, то есть ошибка конфигурации молча
        съедала бы все проверки разом. Ложным `pass` это больше не
        оборачивается — набор из одних `skip` даёт `indeterminate` (§4.3), —
        но раунд всё равно не доказывает ничего, поэтому падаем громко и до
        запуска процессов.
        """
        if tail_lines < 1:
