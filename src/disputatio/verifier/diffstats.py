...
def _run_git(argv: tuple[str, ...], workdir: Path) -> subprocess.CompletedProcess[str]:
    ...
    Оба пайпа читаются через `communicate()`, поэтому взаимная блокировка
    на заполненном буфере невозможна по построению — а она реальна: 130
    строк usage в stderr вне репозитория ([DESIGN-003], [REQ-005]).
