"""Промпт автора doc-раунда — контуры spec/pair/doc (§5.1, [DESIGN-011]).
...
- `pair` — написать/переработать план по уже сошедшейся спеке; **правка
  ...
  правки;
- `doc` *(вид `document`)* — довести до сходимости ЕДИНСТВЕННЫЙ документ.
  Пары у него нет, возвращаться некуда, и `adopted_findings` у него всегда
  пуст: находки прошлого pair-ревью — механика вида `pair` (P10).
...
_INTRO_BY_CONTOUR: Final[dict[str, str]] = {
    ...
    "doc": (
        "# Doc-раунд: работа автора (контур doc)\n"
        "Ваша задача — довести единственный документ, названный ниже, до "
        "сходимости по чеклисту ревьюера (§5.3 SPEC-002): на первом раунде "
        "написать его по задаче ниже, на последующих — переработать по "
        "замечаниям. Права правки чего-либо ещё у вас нет: диф раунда, "
        "тронувший другой путь, валит детерминированный гейт doc-scope "
        "(§6 SPEC-002). Источник истины — файлы рабочей директории, а не "
        "история диалога: прошлые версии документа сюда не передаются "
        "намеренно (§6.1 SPEC-001)."
    ),
    "pair": (
