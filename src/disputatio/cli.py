...
Один модуль на весь CLI, stdlib `argparse` и ни одной новой зависимости:
подкоманд две, а `main(argv) -> int` делает вход обычной функцией — тест
вызывает её напрямую, не порождая процесса и не завися от того, установлен
ли пакет.
...
1. `preflight(root)` — **до** `bootstrap_session`. Отказ на грязном дереве не
   вправе оставить `.disputatio/` в чужом репозитории ([REQ-010]), а
   единственный способ этого добиться — не создавать каталог, пока проверки
   не прошли.
2. `session_id` печатается **до** первого записанного файла и первой строки
   журнала: сессия, которую пользователь не может назвать, не поддаётся ни
   `disp resume`, ни разбору инцидента.
...
def cmd_run(args: argparse.Namespace, *, now: Callable[[], datetime]) -> int:
    ...
    Порядок обязателен: pre-flight git ([DESIGN-010]) выполняется ДО
    `bootstrap_session`, поэтому отказ на грязном дереве не оставляет
    `.disputatio/` ([REQ-010]).
