...
def _reject_unconvergeable_gates(config: RuntimeConfig) -> None:
    ...
    **Покрыты только статические случаи, и это предел по построению.** Гейт,
    ...
    причиной, причём автор диагностики не видит вовсе: в его промпт идут
    только `fail`-гейты. Лечится это не здесь: остановка после раунда без
    единого выполненного гейта — новое условие §5 со своим кодом причины,
