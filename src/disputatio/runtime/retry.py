...
Драйверы (`disp run`/`resume`, драйвер ревизии пайплайна) признают исходом
сессии только их, и только при терминальной фазе на диске: дефект
оркестратора, поднятый после записи `FAILED`, иначе выглядел бы штатным
исходом и был бы замолчан. Кортеж полон: других путей, на которых ядро
пишет `FAILED` и пробрасывает исключение, нет.
"""
...
def _run_lifecycle_hook(
    ...
    причину значило бы завести второй источник правды о P9. Так же, без
    обёртки, уходит I3 (`RoundImmutableError`): драйверы ловить его не вправе,
    а обёртка в `LifecyclePolicyFailed` сделала бы его исходом. Любое другое
