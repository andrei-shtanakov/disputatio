...
class SnapshotPerimeterIncompatible(DisputatioError):
    """Прерванный ход снят версией с узким периметром P9 (§2 P9, §8.1 шаг 0).

    Не подмена: `pre_turn` без поля `revisions` описывает одну текущую
    ревизию, и доказать неизменность соседних за прерванный ход нечем.
    Достраивать снимок по текущему дереву нельзя — это выдало бы за
    доказательство проверяемое состояние. Отказ без мутаций, код `2`;
    повторный resume той же версией даёт тот же отказ.
    """
