...
def _operator_checklist(table: Any) -> ResolvedChecklist:
    ...
    Детерминизм порядка на всю жизнь пайплайна удостоверен: `resume`
    сравнивает immutable-проекцию живого конфига с версионированным
    доказательством снапшота fail-closed, ДО чтения worktree и любых
    мутаций (`pipeline_resume.PipelineResume._verify_semantics`, SPEC-002,
    TASK-004 очереди WS-disputatio-65) — конфиг, изменённый между
    запусками, останавливает `resume` (`errors.SemanticDrift`), а не
    доезжает до ревьюера молча. Ограничение issue #65 закрыто для обоих
