...
class StatusEntry:
    ...
    **`path` — относительно toplevel репозитория, а НЕ относительно `root`
    сессии, и потребитель обязан приводить к нему свои пути сам.** Порцелан
    отвечает от toplevel всегда: локальный `status.relativePaths=true` на
    `--porcelain` не влияет (проверено), а `preflight` совпадения `root` с
    toplevel не требует — комментарий к `_TREE_PATHSPEC` в этом же модуле
    прямо на это опирается.

    Отсюда две ловушки для правил §3.1 SPEC-002, и обе тихие. Сессия в
    подкаталоге `proj/` чужого репозитория отдаёт
    `proj/.disputatio/pipelines/…` и `proj/spec/pair.md`, поэтому:

    * фильтр вида `path.startswith(".disputatio/")` не совпадёт ни с чем, и
      предусловие старта отвергнет **легальный** запуск по собственному
      журналу пайплайна;
    * сравнение с `spec_path`/`plan_path`, если те заданы от `root`,
      промахнётся, и scope adoption'а отклонит сам документ пары.

    Правильный приём — привести обе стороны к одной базе (префикс `root`
    относительно toplevel), а не подпирать сравнение эвристикой по хвосту
    пути.
...
class GitOps(Protocol):
    ...
    Четыре первых метода — цикл раунда SPEC-001. Семь остальных пришли с
    операторскими решениями SPEC-002 (§3.1), cleanup'ом возврата (§7.3),
    сверкой worktree на resume (§8.1) и предусловиями `run` (§3.1, задача 13):
    у них нет другого пути к git, а `subprocess` мимо порта был бы вторым
    слоем доступа к репозиторию (INV-11) — с собственным окружением,
    собственной идентичностью и собственными представлениями о том, что
    считать каноническим диффом.
    """
    ...
    def toplevel_prefix(self) -> str:
        """Путь `root` относительно toplevel репозитория (SPEC-002 §3.1).

        `StatusEntry.path` приходит от toplevel всегда, а `spec_path`/
        `plan_path`/`slug` конфига и предусловий — от `root` сессии; когда
        `root` не совпадает с toplevel (сессия в подкаталоге чужого
        репозитория), сравнение без этого префикса промахивается в обе
        стороны — см. докстринг `StatusEntry`. Порт не выдумывает
        нормализацию сам (`status_entries` отдаёт сырой toplevel-путь
        намеренно): префикс — отдельная операция, а не скрытый побочный
        эффект другого метода.

        Пустая строка — законный ответ: `root`, совпадающий с toplevel, префикса
        не имеет. Иначе результат оканчивается `/`.
        """
        ...
...
class GitCli:
    ...
    def status_entries(self) -> tuple[StatusEntry, ...]:
        ...
        безвозвратно. Фильтрация — обязанность потребителя, и **базу путей
        он обязан привести сам**: они идут от toplevel, а не от `root`
        сессии (`StatusEntry` — чем это грозит наивному фильтру).
        ...
        `--no-renames` не косметика, и не только из-за видимости путей.
        Свёрнутое переименование `R new` прячет исходный путь внутрь одной
        записи, и fail-closed scope §3.1 не увидел бы, что кроме документа
        пары из дерева исчез посторонний файл. Хуже того, ломается сам
        разбор: второе поле такой записи (`-z` кладёт его следующим) идёт
        БЕЗ кода `XY`, и срез `record[3:]` съел бы у него три первых
        символа, выдав мусорный путь с `tracked=True`. Без свёртки git
        называет обе половины отдельными записями (`D old`, `A new`), каждая
        со своим кодом, и обе проходят проверку области.
        """
        raw = _checked(
            self.root,
            "status",
    ...
    def commit_paths(self, paths: Sequence[str], subject: str, *, trailer: str) -> str:
        ...
        **Ограничение области: путь под `.disputatio/` закоммитить нельзя.**
        Со второго принятого раунда каталог сессии лежит в
        `.git/info/exclude` ([DESIGN-011]), а `git add` с явно названным
        игнорируемым путём считает это ошибкой и выходит кодом 1 (`The
        following paths are ignored`) — метод отдаст `GitCommandError`.
        Нормам §3.1 это не мешает: операторский чекпоинт фиксирует только
        `spec_path`/`plan_path`, а журнал оркестратора в историю попадать и
        не должен. Но порт объявлен общим, поэтому ограничение названо здесь,
        а не выводится потребителем из падения: обойти его можно лишь
        `--force`, и передавать его молча метод не будет — коммит журнала
        сессии в рабочую историю пользователя требует отдельного решения,
        а не флага внутри adoption'а.
    ...
    def toplevel_prefix(self) -> str:
        """`root` относительно toplevel репозитория (SPEC-002 §3.1).

        `git rev-parse --show-prefix` — единственная команда порта, отвечающая
        именно этим: она даёт путь `cwd` (== `root`) относительно toplevel, с
        завершающим `/`, и пустую строку, когда `root` и есть toplevel.
        `--show-toplevel` не годится на замену — он отдаёт АБСОЛЮТНЫЙ путь
        toplevel, а сравнивать предстоит ОТНОСИТЕЛЬНЫЕ пути `StatusEntry`.

        `.strip()` снимает только завершающий перевод строки: сам префикс
        пробелов не содержит, а обрезать его целиком — значит потерять
        разделитель, на котором держится сравнение (`f"{prefix}{name}"`).
        """
        return _checked(self.root, "rev-parse", "--show-prefix").strip()
