...
class PipelineRunner:
    ...
    def _do_record_return(
        ...
        Два шага, и оба идемпотентны. `reset --hard` к последнему принятому
        коммиту — это HEAD: принятые раунды закоммичены, а припаркованный не
        принят, поэтому повтор сброса к тому же коммиту — no-op. Commit point
        — ровно ОДНА запись манифеста, несущая всё сразу: transition,
