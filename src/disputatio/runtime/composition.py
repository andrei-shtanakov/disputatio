...
def _require_toplevel(git: GitOps, workspace: Path) -> None:
    ...
    остаётся. Норма записана в SPEC-002: §3.1 делает «`--root` — toplevel
    репозитория» первым предусловием `run` (отказ до первой мутации), а §6
    называет базой сравнения путей `doc-scope` корень репозитория и прямо
    говорит, что нормализации «от `--root` к toplevel» у гейта нет — её
    заменяет это предусловие. Снять ограничение, дав `_doc_verifier` тот же
    `toplevel_prefix`, значит сначала поменять обе нормы.
    """
