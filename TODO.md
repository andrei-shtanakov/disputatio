...
- [ ] Прогон-доказательство миграции @id:tdd-gate-migration-proof
  ...
  **Прогон 1 (2026-08-22) — FAILED, находка засчитана.** Цепочка построилась
  целиком: декомпозиция дала ровно одну leaf-задачу, RED лёг в
  `tests/verifier/test_task_001_red.py` (инструкция сработала, страховочный
  глоб не понадобился), red подтверждён реплеем, claim активен, фазы включая
  `refactoring`, гейты `tdd.red`/`tdd.claims` — satisfied, ревью прошло
  (`REVIEW_FIXED`), хук `pyrefly` — succeeded в нужном порядке. Упал
  единственный шаг — наш экспортёр: «неполная цепочка для TASK-001:
  gate-verdict:review». Причина не в spec-runner: под дефолтным
  `review_policy: advisory` гейт ревью **не регистрируется вовсе**
  (`gates.register_builtin_gates` у соседа), то есть экспортёр требовал
  вердикт, которого конфигурация не производит. Наш дефект, чинится
  `review_policy: required`. Поскольку ключ входит в `POLICY_KEYS`, прежние
  вердикты протухают — нужен чистый прогон, а не продолжение. Побочно
  подтвердились два обещания: post-mortem архив maestro сохранил БД вне
  worktree (по ней и восстановлена картина после того, как retry затёр
  worktree), а namespace в БД оказался хешем `d4ff62d4adb76ae1` — ровно тот
  случай, ради которого экспортёр выводит `ws-<id>` сам.
