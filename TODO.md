...
- [ ] Автоматизация полировки пары спека+план протоколом раундов @id:spec-pair-polish-automation
  ...
  **не публиковался**: штатный путь отказал, отпечаток не вычислен, ручное
  ...
  (`f7a03d6`) не трогать. Починены 4 из 13 — три major (C3 `d0703cb`,
  A5 `836330b`, D1 `27791f6`) и единственный minor (D3 `677f1dd`), suite 1814.
  Открыты 9 major (12 major = 3 починенных + 9); порядок: (1) общий парсер с
  состоянием fence — B1+B2+A4, один корень; (2) группа A под принципом
  «непонятное состояние не считается успешным»; (3) C, затем D2. Повторное
