...
- [ ] Гейт «объявлено, но не подключено» @id:gate-declared-not-wired @epic:eco.disputatio
  ...
  **в боевом коде** — `runtime/composition.py:531`; тест-стенды конструируют
  ...
  §10 п.2** (`disputatio-SPEC-001-round-protocol.md:283`), а не §11 SPEC-002,
  как сказано в issue #60: в §11 SPEC-002 семь буллетов и ни одного про
  ревьюеров — третья поправка к тексту issue. Прогон issue называет основанием
  пересмотреть; арбитра не трогать, возможен erratum к
  §10 SPEC-001, явно различающий разногласие (лечится арбитром) и слепое пятно
