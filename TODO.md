...
- [ ] Гейт «объявлено, но не подключено» @id:gate-declared-not-wired @epic:eco.disputatio
  ...
  `runtime/composition.py:531`) и `doc_sessions` не попала в append-only guard,
  ...
  Смежное: §11 SPEC-002 держит «несколько ревьюеров / агрегацию вердиктов» вне
  v1 — issue называет
  этот прогон основанием пересмотреть; арбитра не трогать, возможен erratum к
