...
- [x] Оценка миграции TDD-гейта на штатный режим @id:tdd-gate-migration-assessment
  ...
  не уничтожается (post-mortem архив #164 невыключаем), но перестаёт быть
  трекаемым содержимым репо и становится машинно-локальным архивом с ротацией
  `keep_per_workstream: 5` — не мержится, не видна в ревью. Рекомендация —
  гибрид: принять штатный гейт, заменив 1997 строк не нулём, а экспортёром
  evidence в трекаемые файлы.
- [ ] Решение по миграции TDD-гейта @id:tdd-gate-migration-decision
  — принять или отклонить рекомендацию оценки (§8 отчёта). Предложенный
  порядок, если принимаем: конфиг → конституция (п. 2–4 инструктируют звать
  `tdd_gate.py red`; в штатном режиме от агента нужен маркер `TDD_SELECTOR:`)
  → экспортёр evidence → прогон-доказательство на одной leaf-задаче →
  и только потом удаление `scripts/tdd_gate.py` (1997), `tests/harness/`
  (2725, 130 тестов), `spec/plugins/tdd-gate/`. Неотвеченный вопрос оценки —
  откуда экспортёр читает: живая БД в worktree (вероятно, нужны правки у
  соседа) или post-mortem архив (не нужны).
...
- [x] maestro: `validate --strict` не эскалирует warnings @id:maestro-strict-warnings-finding
  ...
  пункта нет тега `@blocked_by:todo://maestro/163` — он жил только под
