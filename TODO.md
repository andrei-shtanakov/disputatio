...
- [x] INV-12 нарушен: maestro двигает локальный master @id:inv-12-local-master-fast-forward
  ...
  **Разобрано по коду соседа, решение принято.** Ключа нет:
  ...
  `origin/master`, при расхождении `reset --hard`». Риск тихой ошибки здесь
  низкий: после мержа PR локальный master расходится с origin, и
  `git pull --ff-only` отказывает громко — что и произошло.
