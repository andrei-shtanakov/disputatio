...
- [ ] Гейт «объявлено, но не подключено» @id:gate-declared-not-wired @epic:eco.disputatio
  ...
  CONVERGED по полному P1–P5, — после чего терминальное ревью PR нашло пять
  блокирующих дефектов подряд, из них один архитектурный (возврат
  `PAIR_LOOP → SPEC_LOOP`); транскрипт —
  ...
  лечить не ту болезнь. Главное содержание пункта — детерминированный гейт: три
  из пяти находок были машинно проверяемы без LLM одним `grep`. **Все три в
  дереве уже закрыты возвратом PR #54** — это материал для фикстур по истории,
  а не дефекты HEAD, искать их в текущем коде бессмысленно: `EDGES_BY_KIND`
  объявлялась и не подключалась к валидации (сегодня подключена model-валидатором
  `PipelineState`, `contracts/pipeline.py:611`); `ArchitecturalDefectPolicy`
  конструировалась дважды при объявленном инварианте P10 «только composition
  root» (сегодня единственная конструкция — `runtime/composition.py:531`);
  `doc_sessions` не попала в append-only guard, потому что состав был выписан
  литералами там, где рядом есть готовый источник (сегодня под guard'ом,
  запинено `tests/events/test_pipeline_store.py::test_doc_sessions_guarded_too`).
  Гейт ловит не «плохой дизайн», а именно потерянный шов, подключается как
  обычный doc-гейт сверх baseline (SPEC-002 §6), ядра и runner не касается. Смежное: §11 SPEC-002
  держит «несколько ревьюеров / агрегацию вердиктов» вне v1 — issue называет
