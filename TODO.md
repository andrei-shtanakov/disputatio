...
- [ ] Имя каталога трекаемой evidence @id:tdd-evidence-namespace-dir
  ...
  ради которого выбран гибрид. Варианты: принять хеш; научить экспортёр
  выводить каталог из ветки (`ws/<id>` → `ws-<id>`, как INV-16), оставив сырой
  namespace внутри JSON. Решать до прогона-доказательства — артефакт прогона
  ляжет по выбранному пути.
- [ ] Место для pyrefly в пер-тасковом гейте @id:pyrefly-in-standard-gate
  ...
  на 22 задачи, вскрытый на byte-locked тестах) при этом в силе. Кандидат —
  блокирующий плагин на `post_review`, рядом с экспортёром evidence. Решать
  вместе с прогоном-доказательством.
