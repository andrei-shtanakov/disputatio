...
- [ ] Гейт «объявлено, но не подключено» @id:gate-declared-not-wired @epic:eco.disputatio
  ...
  из пяти находок машинно проверяемы без LLM одним `grep` (`EDGES_BY_KIND`
  объявлена и не подключена к валидации; `ArchitecturalDefectPolicy`
  конструируется дважды при объявленном инварианте P10 «только composition
  root»; `doc_sessions` не попала в append-only guard — состав выписан
  литералами там, где рядом есть готовый источник). Гейт ловит не «плохой
  дизайн», а именно потерянный шов, подключается как обычный doc-гейт сверх
  baseline (SPEC-002 §6), ядра и runner не касается. Смежное: §11 SPEC-002
