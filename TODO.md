...
- [x] Прогон-доказательство миграции @id:tdd-gate-migration-proof
  ...
  **Прогон 2 (2026-08-22) — FAILED на `TASK_BLOCKED`, две находки.**
  Цепочка дошла до GREEN: red подтверждён реплеем (`549a8286`,
  `expected_fail`), claim активен. RED-пас захардкодил в ассерте код возврата
  **128**, а `git diff --numstat HEAD` вне репозитория даёт **129** —
  usage-ошибку fallback'а `--no-index`, а не fatal. Проверено вручную:
  `rc=129`. Red при этом был честным (падал `DID NOT RAISE`), поэтому неверная
  константа была невидима до green.
  *Находка 1 — дисциплина сработала.* GREEN-пас реализовал поведение
  корректно, обнаружил неверную премиссу, **не стал** ни править залоченный
  тест, ни подгонять реализацию под заведомо неверный ассерт (назвал это
  «gaming an assertion I know to be wrong»), остановился с `TASK_BLOCKED` и
  указал нужный remedy — ровно то, что предписывает п.3 конституции. Байт-лок
  и конституция проверены на живом соблазне, а не в теории.
  *Находка 2 — remedy недостижим в Mode-2, handoff соседу.* Применить
  `spec-runner tdd repair` не к чему: автоповтор workstream'а (`failed →
  retrying → decomposing`, 2 секунды после падения) пересоздаёт состояние, и в
  живой `.executor-maestro-state.db` не остаётся ни чекпоинта, ни claims
  (проверено: все TDD-таблицы пусты, а в post-mortem архиве того же момента
  чекпоинт и claim на месте). Отключить автоповтор из `project.yaml` нельзя:
  `max_retries` есть только в runtime-модели `Workstream`, во входном
  `WorkstreamConfig` такого поля нет. Итог: операторские remedy, которые мы же
  и заказывали в D7-A, под maestro недостижимы — заведено issue в maestro.
...
- [x] Сквозной операторский remedy-путь под maestro @owner:repo:maestro @id:operator-remedy-path-under-maestro @trigger:"maestro выпущен с фиксом TASK_BLOCKED (maestro#212)" @epic:eco.disputatio — пройден 2026-09-26, транскрипт `docs/plans/2026-09-26-remedy-path-proof-transcript.md`
  ...
  состояние раньше (`todo://disputatio/maestro-retry-wipes-tdd-state`, сосед
  закрыл своим PR #212, но релиза с фиксом нет: установлен maestro 0.4.0 без
  него). Остаток не исчез вместе с закрытием `tdd-gate-red-supersede`: команды
  соседа объявлены заменой нашего `--supersede`, но сквозным путём под maestro
  ...
  **2026-09-25: условие выполнено без релиза.** Тегового релиза с фиксом нет
  (теги `v0.1.0`/`v0.2.0`, версия в `pyproject.toml` соседа — 0.4.0), но
  установленный `uv tool` maestro переустановлен из master соседа: коммит
  `e659eac` (2026-09-21) содержит мерж #212 `bdc2a83`, в
...
- [x] Автоматизация полировки пары спека+план протоколом раундов @id:spec-pair-polish-automation
  ...
  наблюдаемый признак готовности запроса — работающий `disp pipeline` — выполнен.
  — **к merge допущено не сразу.** Решением владельца 2026-08-30 PR был
  заблокирован: терминальное ревью нашло **13 находок (12 major + 1 minor)**,
  из них пять — fail-open в защитах, ради которых репо существует. Кит
  `review-pr.sh` отказал на цельной ветке по размеру (104 файла / 1.19 МБ против
  потолков 30 / 400 КБ; chunked-режим не реализован,
  `@id:review-kit-large-pr-mode`), поэтому ветка прогнана
  двенадцатью срезами по границам задач через `scripts/review/local.sh
  --base/--head` — сегменты стыкуются без разрывов, покрытие полное. Вердикт
  **не публиковался**: штатный путь отказал, отпечаток не вычислен, ручное
  ревью от своего имени запрещено владельцем. Находки —
  `docs/plans/2026-08-30-codex-terminal-review-pr51-findings.md`.
  Исправления — веткой `fix/codex-findings-pr51`, базовый диапазон PR
  (`f7a03d6`) не трогать. **Все 13 закрыты 2026-08-30**, suite 1803 → 1911.
  Финальное терминальное ревью фикс-ветки пройдено тремя срезами: одна
  находка — регрессия в фиксе A3, потребовала ещё двух раундов на анкере
  (`14667ff` усечение хвоста, `47f7747` сериализация), повторный прогон чист.
  Отдельного фикс-PR в итоге не понадобилось: исправления влиты в ту же ветку
  `feat/spec-pair-pipeline` и приехали вместе с PR #51. Сверх находок под
  порогом владельца приняты два симметричных места (`45843ff` пропущенный
  baseline-гейт давал зелёный отчёт, `1ff896b` тот же дефект C1 в экспорте
  сессии); одно отозвано revert'ом `b99262b`. Пять открытых вопросов владельцу —
  в findings-документе, из них главный: `write_result` обещал «сбой оставляет
  прежний манифест», обещание было дефектом и заменено на «marker снят»
  (единственное изменение зафиксированного поведения за цикл); нормативный
  текст догнал код отдельным docs-коммитом — §8.2 SPEC-002 и докстринги
  `events/{export,atomic}.py`, `runtime/pipeline_export.py`. Планировавшийся
  повторный прогон штатным `review-pr.sh` на отдельном фикс-PR не состоялся —
  фикс-PR не заводился (см. выше); ветка ушла в master после срезового прогона.
  Вендорённый кит не править (пин `steward @ e4c43cc`), нужен chunked — issue
  в `steward` по ADR-ECO-006.
  — **урок метода:** внутренний цикл (17 задач + ревью ветки + мутации) не
  нашёл ни одной из 12 находок. Все они на стыках компонентов, каждый из
  которых по отдельности покрыт тестами. Мутации измеряют прочность
  существующих тестов и структурно слепы к сценарию, для которого теста нет.
  Тесты писались от того, что код умеет разбирать, а не от того, из чего
  состоят реальные входные данные (fenced code, бинарники, повреждённые записи).
  — реализация: 45 коммитов, 17 задач плана,
  suite 1124 → 1803. Пять пакетов: contracts (схема disputatio/v2,
  disputatio/pipeline/v1, валидация V1–V8, три порта), events (хранилище с
  prefix-equality guard, анкер целостности вне дерева), verifier (пять
  неотключаемых doc-гейтов), context (промпты контуров за барьером инъекции),
  runtime (разделение корней, семь операций GitOps, конфиг, экспорт, runner,
  resume с P9 и операторскими решениями, CLI `disp pipeline`). Ядро FSM не
  тронуто. Описание PR обещало отдать восемь остаточных находок
  отслеживаемыми issue, а девять расхождений с текстом SPEC-002 — владельцу
  спеки; ни того, ни другого сделано не было, обе кучки ведёт пункт
  `todo://disputatio/pr51-residual-findings` ниже.
  — дизайн и план: PR #50: `disputatio-SPEC-002-doc-pipeline.md`
  + `docs/plans/2026-08-28-spec-pair-pipeline-plan.md` (17 задач). Спека
  прошла 6 раундов ревью до spec converged, пара — 6 до pair converged;
  первый прогон правила `spec-authoring` в этом репо (один PR на пару).
  Ключевое из ревью: якорь целостности P9 переехал с несуществующей
  adapter-гарантии на файловую границу вне рабочего дерева; предусловие
  `run` сужено, иначе `clean()` первого `PROPOSING` уничтожал бы файлы
  пользователя; `GitOps` расширяется пятью операциями. Реализация — после
  мержа.
  — inbox-запрос #44 (принят 2026-08-28) по правилу флота
  `../prograph-vault/authored/rules/spec-authoring.md` (prograph-vault#102):
  дефолт «спека и план полируются локальным циклом author↔reviewer до
  сходимости, PR один — на готовую пару» сейчас исполняется вручную (Codex CLI
  в соседнем терминале). Просьба — автоматизировать протоколом SPEC-001 два
  вложенных контура: `author ↔ spec reviewer` → converged →
  `author ↔ spec-plan reviewer` → converged → один PR. Условия сходимости
  контуров различны; редакция на момент приёма (из issue, SSOT — правило):
  **spec converged** = нет blocker/major, нет TBD/открытых архитектурных
  вопросов, ошибки/authority определены, интерфейсы и не-цели согласованы,
  факты сверены по коду; **pair converged** = трассируемость спека→план, план
  не вводит новых решений, порядок сохраняет рабочее состояние, тесты
  доказывают требования включая fail-closed, команды выполнимы. Перед
  реализацией свежую редакцию правила вендорить пиненой копией внутрь.
  Возврат: архитектурный дефект на ревью пары → к спеке с полной перепроверкой
  плана, замечание об исполнении → только план. Не срочно — после текущей
...
- [x] Режим полировки одиночного документа @id:single-document-polish-mode
  ...
  **Оба хвоста закрыты 2026-09-01.** Inbox-issue #52 закрыт как COMPLETED
  ...
  markdown-документа (charter / requirements / behaviour-spec с DSL
  `#### BEH-NN` + `traces:` + `- **checked_by**:`). Признак готовности от
  заказчика — документированный CLI-способ полировать один документ; после него
  devtools переключит узел авторинга с `run --mode develop` на него.
  — **начинаем не с нуля.** После PR #51 в master уже есть: `Mode.DOCUMENT`
  в contracts, пять неотключаемых doc-гейтов, чеклист spec-контура S1–S5 с
  override через `[pipeline.checklists.spec]`, resume с анкером целостности P9,
  экспорт. Не хватает ровно трёх вещей: `plan_path` в `[pipeline]` обязателен;
  FSM всегда идёт `SPEC_LOOP → PAIR_LOOP`, ребра `SPEC_LOOP → EXPORTING` нет;
  контур — закрытый enum `spec|pair` (`CHECKLIST_BY_CONTOUR`), и чеклист
  привязан к нему по имени, тогда как чартеру нужен свой набор условий.
  — форму выбирает владелец, на столе три:
  **(а)** одноконтурный пайплайн — `plan_path` необязателен, сходимость
  единственного контура терминальна; переиспользует всё написанное;
  **(б)** открыть `disp run --mode document` наружу — дёшево, но без чеклистов,
  doc-гейтов и экспорта, то есть без того, ради чего режим и заводился;
  **(в)** отказ «develop достаточно», закрыть not planned — заказчик назвал этот
  исход валидным.
  Склонность — (а): она не заводит второй механизм под ту же дисциплину. Это
  правка нормативного текста SPEC-002 (§2 FSM, §3.2 конфиг, §5.3 привязка
  чеклиста к контуру), значит идёт парой «спека + план» по правилу
  `spec-authoring` — и первым дегфудингом влитого `disp pipeline` на самом себе.
  — §11 SPEC-002 отложила обобщение до N-stage «до второго реального
...
- [x] `resume` сверяет только вид: дрейф конфига относительно снапшота @id:resume-config-drift-vs-snapshot — закрыт WS-disputatio-65 (issue #65, TASK-001–TASK-004, PR #90–#93): immutable-проекция + semantic comparison на resume, вид манифеста сверяется с доказательством (P0)
  ...
  тогда как манифест удостоверяет sha256 снапшотов `config.toml` и
  `checklists.toml`. Значит на `resume` автор правит и ревьюер судит по
  поданным сейчас путям и критериям, а не по тем, чей хеш записан.
  **Относится к обоим видам** — `pair` и `document`; у `document` радиус
  шире, потому что конфиг задаёт весь состав и порядок чеклиста, а не
  только тексты.
  — **дефект старше ветки:** существует с PR #51; PR #64 его не создавал —
  до ветки `resume` не сверял с конфигом вообще ничего, ветка добавила
  сверку вида, то есть сузила поверхность и обнаружила остаток.
  — **чинится не патчем.** Полная сверка живого конфига со снапшотом
  запретила бы легитимную правку runtime-настроек (`soft_max_pipeline_*`,
  `protected_branches`, добавленные гейты); чтение снапшота требует модели
  доверия к файлу в АВТОРСКОМ дереве, которому §8.1 шаг 0 прямо не
  доверяет. Вероятное решение — разделить секцию на immutable session
  semantics (пути документов, состав/порядок/роль чеклистов, гейты) и
  mutable resume/runtime controls, сверять fail-closed только первую и
  сказать это в нормативном тексте. Это правка SPEC-002 (§3.2, §8.1).
  — **обязательное продолжение, а не пожелание:** блокирует дальнейшую
  реализацию `resume` для вида `document` сверх влитого PR #64 — наращивать
  поверхность поверх непринятого решения нельзя. До закрытия
  durable-воспроизводимость resume относительно чеклистов и путей
  документов не обещается (известное ограничение объявлено в
...
- [x] Control-plane не защищён в SSOT @id:control-plane-not-in-ssot @epic:eco.disputatio — закрыт 2026-09-24: `spec-runner.config.yaml` и `pyrefly.toml` внесены в `harness_files` `project.yaml` (сверено по коду spec-runner: guard хеширует файлы на диске, неотслеживаемый сгенерированный конфиг покрыт; maestro пишет его до запуска); расхождение doc §2 ↔ SSOT запинено `tests/test_project_harness.py`
  ...
  политику). В `project.yaml`, который в Mode-2 и порождает worktree-конфиг,
  их нет и не было — значит на живом прогоне ни политика, ни послабления
  typecheck не защищены, а документация утверждает обратное. Дефект старше
  сноса гейта (найден ревью PR #40), поэтому в атомарный PR удаления не
  вносился. Чинить в SSOT, а не в доке: обещание должно стать правдой, а не
  исчезнуть из текста.
- [x] INV-12 нарушен: maestro двигает локальный master @id:inv-12-local-master-fast-forward
  ...
  **Разобрано по коду соседа, решение принято.** Ключа нет:
  `orchestrator._merge_into_base` вызывается безусловно перед DONE, и сам
  DONE гейтится успешным мержем («чтобы следующий workstream видел
  предыдущую работу»). Больше того, `base_branch` управляет двумя вещами
  сразу и развести их нельзя: он же идёт в `gh pr create --base`
  (`pr_manager`). Значит «интеграционная ветка» не бесплатна — PR-ы
  workstream'ов уедут в неё, и чтобы evidence попала в master, понадобится
  второй, ручной PR.
  Перечитан исходный INV-12: он требовал интеграционную ветку не вообще, а
  потому что **семь** workstream'ов волны не должны лить семь PR в master.
  Для одиночного прогона верно обратное — PR обязан целиться в master, иначе
  код и evidence минуют обычное ревью.
  Поэтому принято двухрежимное правило (записано в шапке `project.yaml` и в
  git-workflow `CLAUDE.md`): волна → интеграционная ветка, как в волне 1;
  одиночный прогон → `master` плюс обязательный пост-шаг «сверить `master` с
  `origin/master`, при расхождении `reset --hard`». Первая редакция правила
  опиралась на довод «ошибиться тихо нельзя, `pull --ff-only` откажет» — довод
  неверный и снят по ревью PR #41: пока PR не смержен, локальный master просто
  ВПЕРЕДИ origin, и pull отвечает «Already up to date» с кодом 0 (проверено на
  синтетическом репо). Громкий отказ приходит лишь после мержа PR. Значит окно
  тишины реально и приходится ровно на момент после прогона — сверка SHA
  обязательна, а не «на всякий случай».
- [x] `tdd_gate red --supersede` — v2 гейта @id:tdd-gate-red-supersede
  (осознанная замена red-эталона вместо ручного вмешательства оператора).
  **Снят вместе с гейтом 2026-08-22**: разрабатывать v2 удалённого инструмента
  нечего. Сценарий закрывают штатные команды соседа —
  `spec-runner tdd abandon` (этот red негоден, коммит остаётся в истории,
  разрешён новый честный цикл), `spec-runner tdd repair` (правка залоченного
  файла законна, открывается новая родословная с provenance),
  `spec-runner tdd resume` (зелёное достигнуто — вернуть подтверждённый red и
  claims одной транзакцией), `spec-runner tdd release` (задача завершена —
  снять локи; в прогоне 3 отработал сам: на `post_review` claim был `active`,
  а в post-mortem архиве, снятом после DONE, он уже `released`). Оценка
  предсказывала это в §4.4; подтверждено прогоном 2, где
  сценарий возник по-настоящему: агент упёрся в залоченный тест с неверным
  ассертом и сам назвал `repair` нужным remedy. Оговорка: применить его тогда
  помешал автоповтор maestro (`todo://disputatio/maestro-retry-wipes-tdd-state`,
  у соседа закрыто их PR #212, у нас не перепроверено) — то есть команды
  существуют и уместны, но сквозной операторский путь под maestro ещё не
  проходили.
...
- [x] Агрегатор не вправе возвращать `pass` без единого фактического `pass` @id:aggregate-no-vacuous-pass @epic:eco.disputatio
  — issue #56, нормативное решение владельца от 2026-08-31 (заведением пункта
  не пересматривается). `verifier/aggregate.py::compute_overall` отдаёт `pass`
  и для пустого набора гейтов, и для набора из одних `skip`: единственная
  проверка — `any(status == FAIL)`, иначе `PASS`. Это fail-open в самом
  приборе — зелёный вердикт там, где не выполнено ни одной проверки, а ложный
  `pass` открывает дорогу к `CONVERGED` ([REQ-008], сказано прямо в докстринге
  функции). Правило: сходимость требует **хотя бы одного фактически
  выполненного `pass`** при отсутствии `fail`; пустой набор и набор из одних
  `skip` — не `pass`. Представление результата (`fail`, отдельный
  `indeterminate` или ошибка конфигурации) — часть design-работы пункта, а не
  блокер перед его заведением; единственное ограничение — наружу поведение
  fail-closed. Правка нормативная, а не багфикс: критерий сходимости зафиксирован
  SPEC-001 [DESIGN-009] и общий для всех режимов, значит спека переписывается
  первой. Отдельным PR, не смешивая с реализацией document pipeline: в общем PR
  пункт теряет и трассируемость, и критерий готовности. Проверить заодно, что
  baseline-гейт слоем выше (`45843ff`) не маскирует поведение агрегатора в
  тестах — дыра doc-контура закрыта там, сам агрегатор оставался fail-open.
  **Выполнено PR #97.** Представление выбрано отдельным значением
  `indeterminate`: наружу fail-closed, но «проверка провалилась» и «проверок
  не было» в артефакте больше не одно и то же. §4.3 SPEC-001 переписан,
  правило стало трёхступенчатым; карман §5.1 п.2 (`analyze` с пустым набором)
  до правки был мёртвым кодом — пустой набор давал `pass` в любом режиме.
  Побочно вскрылось, что вакуумной сходимостью пользовались собственные
  e2e-фикстуры CLI (профиль без гейтов в режиме `develop`) — им выдан
  настоящий пустой гейт `true`. Проверка из критериев готовности выполнена
  отдельным пином: `skip` у `extra`-гейта не роняет doc-отчёт с зелёным
  baseline. Замечено и не сделано: тег `[DESIGN-009]` в докстрингах
  агрегатора указывал не туда (в разложении волны 1 это «Учёт бюджета —
  `runtime/budget.py`»); ссылки в тронутых местах переставлены на §4.3
  SPEC-001, сплошная ревизия тегов по репо — отдельный пункт, если заводить.
- [x] У журнала один писатель: записать ограничение и назвать компонент @id:journal-single-writer-normative @epic:eco.disputatio — закрыт 2026-09-24 решением владельца (вариант 2): SPEC-002 P11 — один пишущий `disp` на каталог, обеспечивает операторская дисциплина; `exclusive_lock` — дополнительная защита, не гарантия; докстринги `_append`, `save`, `events/file_lock` приведены; К3 обоснован ролью журнала (P8), не составом потребителей
  — issue #58, нормативное решение владельца от 2026-08-31: конкурентная запись
  вне поддерживаемой модели. Пункт закрывается текстом, а не кодом, но снять
  надо реальное расхождение, а не оформить формальность. Findings ставили
  вопрос как «`IntegrityAnchor._append` — read-check-append без сериализации»;
  **на master это уже не так**: `events/integrity_anchor.py:219 _append` держит
  чтение, решение о хвосте, усечение и запись под одной `exclusive_lock`
  (`events/file_lock.py`, фикс `47f7747`, приехавший в ветке позже, чем писался
  findings-документ), и докстринг обосновывает блокировку конкретной гонкой —
  два `disp pipeline resume` над журналом с оборванным хвостом снимают один и
  тот же offset, второй дописывает `pre_turn`, первый срезает её как хвост.
  Код сериализует конкурентных писателей, допущение объявляет их вне модели —
  обоснованием оба утверждения одновременно быть не могут. Развилка: либо
  модель допускает несколько процессов над одним журналом, и тогда
  `events/file_lock.exclusive_lock` и есть обеспечивающий компонент («писатель
  ровно один в каждый момент, сериализацию обеспечивает …»), либо не допускает,
  и тогда блокировка описывается как эшелонированная защита, а в спеке сказано,
  что́ обеспечивает единственность писателя вместо неё. Отдельным основанием
  идёт К3: `events/pipeline_events.py:148 read_pipeline_events` молча пропускает
  нечитаемые строки и оборванный хвост, а не-UTF-8 файл отдаёт пустым списком.
  К3 частично пересекается с `todo://disputatio/pr51-residual-findings`, где
  «`read_pipeline_events` без потребителя» числится среди восьми мест из
  описания PR #51 — там это учётная запись, здесь нормативный вопрос.
  Довод «внешних потребителей у журнала нет» обосновывает К3 и только его: с
  допустимостью single-writer он не связан — сегодня потребителей нет, завтра
  появятся, а нормативное ограничение переживает их состав.
- [x] Читаемость цели ссылки — защищаемое свойство doc-гейта или нет @id:doc-anchor-unreadable-target @epic:eco.disputatio — закрыт 2026-09-24 решением владельца: вне защищаемых, SPEC-002 §6 «Читаемость цели — вне гарантий»; гейт и тесты не менялись
  — issue #59 (открытый вопрос владельцу, кандидат К1 списка
  `docs/plans/2026-08-30-codex-terminal-review-pr51-findings.md:223`).
  `gate_doc_anchors` молчит, когда цель ссылки существует, но нечитаема
  (например, снятые права на чтение). Попытка это чинить была и **отозвана**:
  коммит `0914073` → revert `b99262b`, потому что достижимость случая в контуре
  не была доказана тестом. Пункт закрывается **выбранным ответом**, а не
  рассуждением: неформального довода «нечитаемость прав в реальном контуре
  невидима» недостаточно — это утверждение о модели угроз, оно либо становится
  записанным нормативным ответом, либо не годится. Свойство защищаемое ⇒ назван
  компонент, который его обеспечивает (`gate_doc_anchors` или integrity
  anchor), и `chmod`-тест доказывает срабатывание в контуре — ровно то, чего не
  хватило коммиту `0914073`. Свойство вне защищаемых ⇒ в SPEC-002 явной строкой
  записано, какой класс входов гейт не различает и почему это допустимо; тест
  тогда не требуется. Промежуточное состояние «вроде бы неважно, отката
  хватило» — это и есть то, что пункт закрывает.
- [x] Гейт «объявлено, но не подключено» @id:gate-declared-not-wired @epic:eco.disputatio — пара «спека + план» (вариант A, решение владельца 2026-09-24) в PR #109: `docs/specs/2026-09-24-wiring-gate-design.md`, `docs/plans/2026-09-24-wiring-gate-plan.md`; реализация — по плану после мержа пары. Закрыт: реализован в ветке feat/wiring-gate (PR #110), спека — `docs/specs/2026-09-24-wiring-gate-design.md`, план — `docs/plans/2026-09-24-wiring-gate-plan.md`, приёмка на снимке 23c9297.
  — issue #60. Цикл полировки пары SPEC-002 v0.2 + план (PR #54) прошёл 11
  раундов author↔reviewer и объявил обе сходимости — spec CONVERGED, pair
  CONVERGED по полному P1–P5, — после чего **пять терминальных ревью PR подряд
  дали семь блокирующих находок** (2+2+1+1+1 по разделам транскрипта), из них
  одна архитектурная — возврат `PAIR_LOOP → SPEC_LOOP`. Формулировку issue #60
  «пять блокирующих дефектов» поправляю здесь: пять — это прогонов ревью, а не
  дефектов, и знаменатель метрики пункта считается от семи. Транскрипт —
  `docs/plans/2026-08-31-document-pipeline-review-transcript.md`. Диагноз: это
  не дедлок, а общее слепое пятно. Разногласия не было ни разу — ревьюер
  говорил «почини», автор соглашался и чинил; дефекты пропущены не потому, что
  спор не разрешился, а потому что обе стороны сверяли документ с документом,
  тогда как дефекты жили в связи документов с кодом, которого никто не менял и
  потому не перечитывал. Видно по истории раундов: находки прогонов, где
  ревьюера явно просили «проверь по коду», — архитектурные; раунды
  документ-против-документа давали бухгалтерию плана. **Арбитр SPEC-001 (§5.5,
  §10 вопрос 3) эту поломку не лечит:** он спроектирован как разрешитель
  разногласия на `ESCALATED` — две позиции, вердикт третьего, — а здесь обе
  позиции совпадали и обе были неверны; подключать его «на несходимость» значит
  лечить не ту болезнь. Главное содержание пункта — детерминированный гейт по
  классу «объявлено, но не подключено»: находки этого класса машинно проверяемы
  без LLM одним `grep`. **Все они в дереве уже закрыты** — это материал для
  фикстур по истории, а не дефекты HEAD, искать их в текущем коде бессмысленно.
  Из семи находок терминальных ревью к классу относятся две:
  `ArchitecturalDefectPolicy` конструировалась дважды при объявленном инварианте
  P10 «только composition root» (третье ревью; сегодня единственная конструкция
  **в боевом коде** — `runtime/composition.py:531`; тест-стенды конструируют
  политику законно и по предписанию плана — `tests/runtime/_pipeline_stand.py:441`,
  `tests/runtime/test_pipeline_runner.py:434,479,814`, — так что `grep` гейта
  обязан различать `src/` и `tests/`, иначе фикстура даст ложное расхождение)
  и `doc_sessions` не попала в append-only guard,
  потому что состав был выписан литералами там, где рядом есть готовый источник
  (четвёртое ревью; сегодня под guard'ом, запинено
  `tests/events/test_pipeline_store.py::test_doc_sessions_guarded_too`). Третий
  образец класса — `EDGES_BY_KIND` объявлялась и не подключалась к валидации
  (сегодня подключена model-валидатором `PipelineState`,
  `contracts/pipeline.py:611`) — взят **не** из терминальных ревью: его нашёл
  раунд 1 pair-контура, то есть сам цикл. Приведён как образец класса, а не как
  пропуск цикла, и issue #60 здесь тоже поправлен: он записывал эту находку в
  терминальные. Находка пятого ревью по той же таблице — другая и к классу не
  относится: в `EDGES_BY_KIND` через `_SHARED_EDGES` входили все `(phase,
  FAILED)`, включая фазы чужого вида.
  Гейт ловит не «плохой дизайн», а именно потерянный шов, подключается как
  обычный doc-гейт сверх baseline (SPEC-002 §6), ядра и runner не касается.
  Смежное: «несколько ревьюеров / агрегацию вердиктов» держит вне v1 **SPEC-001
  §10 п.2** (`disputatio-SPEC-001-round-protocol.md`; номер строки намеренно
  снят — §-ссылка переживает правки текста, а строка 283 уехала на 297 уже от
  соседней правки §4.3), а не §11 SPEC-002, как сказано в issue #60: в §11
  SPEC-002 семь буллетов и ни одного про ревьюеров — третья поправка к тексту
  issue. Прогон issue называет основанием пересмотреть; арбитра не трогать,
  возможен erratum к §10 SPEC-001, явно различающий разногласие (лечится
  арбитром) и слепое пятно
  (лечится сменой входов ревьюера, не третьей инстанцией). Не срочно и не
  входит в PR #54; перед реализацией — обычный порядок репо: пара «спека +
  план» по правилу `spec-authoring`, verifier-стадию смотреть в связке с
  research-bench `bench-verify`.
- [x] Гейт wiring: заголовки с отступом и setext не режут раздел задачи @id:wiring-gate-heading-forms @epic:eco.disputatio
  — **закрыт в PR #110**: граница раздела — любой заголовок 1–3 уровня по
  CommonMark (ATX с отступом ≤ 3 и любым пробелом после `#`, setext 1–2).
  — остаток финального ревью PR #110. `task_sections` признаёт границей раздела
  только ATX-заголовок с колонки 0; заголовок с отступом 1–3 пробела и
  setext-заголовок (`Текст` + строка `---`) рендерятся как заголовки, но раздел
  не обрывают, и ссылка на место из следующего раздела засчитывается
  предыдущей задаче (exit 0 воспроизведён). Нужна нестандартная разметка
...
- [x] Гейт wiring: хвосты после мержа PR #110 @id:wiring-gate-followups @epic:eco.disputatio
  ...
  нормализовать `--src` один раз (в CLI или в `read_snapshot`) и запинить
  тестом «`src` и `./src` дают одинаковый результат». Отказ fail-closed, не
  ложный `pass`, поэтому мерж не блокировал. Заодно: тест на `nonlocal` имени
...
- [x] Верифицированная фаза read-only командой @id:verified-phase-readonly-command @epic:eco.disputatio
  ...
  фиксирует threat model «подмена `{"phase":"DONE"}` обязана ловиться».
  Потребителю нужна терминальная реконсиляция «пайплайн для slug уже
  завершился успешно?» после собственного сбоя (успешный run → сбой потребителя
  до записи completion → повтор). Форма на наше усмотрение: `pipeline status
  --verified` с отказом кодом при tampered или отдельная `pipeline phase`.
  Заодно закрывается расхождение внутри репо — status сегодня доверяет файлу,
  подмену которого наш же контракт объявляет обнаружимой. Признак готовности со
  стороны заказчика: документированный способ получить верифицированную фазу
  без мутации; после этого devtools снимает draft с devtools#106 (терминальная
  реконсиляция disp-бэкенда) и проводит живой смоук.
  **Выполнено PR #98.** По дороге вскрылось, что запрошенной гарантии в анкере
  не существовало: `genesis` не несёт хеша `pipeline.json` (манифест до первого
  хода легитимно переписывается), полная сверка идёт только против `pre_turn`
  — внутри оборванного хода, — а `turn_completed` несёт одну identity. То есть
  у пайплайна в покое фазу не подтверждал никто, и команда «фаза после сверки
  anchor» проверяла бы всё, кроме того файла, из которого фазу и читает.
  Поэтому сделан не флаг, а четвёртый вид записи анкера — `terminal`
  (решение владельца): пишется при входе в `DONE`/`FAILED` и связывает
  идентификатор пайплайна, имя фазы и sha256 манифеста. `disp pipeline phase`
  отвечает **из отметки**, манифест только сверяет; три кода возврата
  (`0` подтверждено / `1` подтверждения нет / `2` подмена), пайплайн не
  двигается ни при какой находке. Сверка хода приведена к `last_turn_record` —
  отметка дописывается после закрытия, в том числе закрытия по подмене, и
  накрыла бы собой `pre_turn`, ослабив fail-closed на втором `resume`.

- [x] Раунд без единого выполненного гейта не останавливает сессию @id:indeterminate-stop-reason @epic:eco.disputatio
  — **Закрыто 2026-09-25:** норма — SPEC-001 §5.2a (спека PR #129); реализация — ветка `feat/indeterminate-stop` (`REASON_VERIFICATION_INDETERMINATE` в `core/deciding.py`, `verification_overall` в манифесте).
  ...
  по себе сессию не останавливает: `decide` доходит до `max_rounds` и отдаёт
  `DEADLOCK` с причиной `max_rounds` — симптом вместо причины. В
  `manifest.json` уходит она же (`exporting` печатает `stop_reason:
  decision.reason`), а `overall` в манифест не выводится вовсе, так что
  оператор видит «упёрлись в лимит раундов», а не «проверок не выполнено».
  Автору не видно и подавно: в его промпт идут только `fail`-гейты
  (`context/sections.py::render_failed_gates_section`), `skip` туда не
  попадает.
  **Отказ на старте в PR #97 пробовали и сняли — это часть материала пункта,
  а не пропущенный вариант.** `_reject_unconvergeable_gates` (в дереве его
  нет — заведён и снят внутри того же PR) отвергал
  статически различимые случаи (пустой набор вне `analyze`; все гейты
  `enabled=false`; гейт с невыполнимой командой) до первого вызова агента, и
  ревью показало цену: вместе с вакуумной сходимостью запрет отбирал у
  gate-less `develop` весь исход — включая частичный экспорт, который
  CLAUDE.md обещает всегда, — а такой запуск законен и живой: devtools гоняет
  `disp run --mode develop` для полировки markdown-документа
  (`todo://disputatio/single-document-polish-mode`), где кодовых гейтов может не быть
  вовсе. Плюс запрет покрывал только `cmd_run`: `disp resume` унаследованной
  сессии шёл мимо него. Значит лечение не в пре-флайте, а в условии
  остановки: оно чинит разом статические случаи, рантайм-скип и `resume`, не
  отнимая ни одного исхода — сессия встаёт на первом раунде без свидетельств
  и экспортирует частичное с честной причиной.
  Предмет решения владельца, а не багфикс: остановка на
  `indeterminate` — **новое условие §5** со своим кодом причины, и её место
  в строгом порядке проверок (§5: converged → budget → oscillation →
  max_rounds) надо назвать явно. Смежное и более дешёвое полумеры-решение,
  которое стоит рассмотреть в том же пункте: выводить `overall` в манифест —
  тогда причина хотя бы читается постфактум. Открытость этого хвоста записана
  нормативно (§4.3), чтобы предел не выглядел недосмотром.
  Сосед предупреждён **до мержа правила**, а не «когда пункт возьмут в работу»:
  devtools#151 (`slug: disp-gateless-develop-no-longer-converges`) — их прогон
  `disp run --mode develop` без кодовых гейтов сходиться перестанет и будет
  доходить до `ESCALATED` с частичным экспортом. Проверить их профиль по
  нашему репо нельзя, поэтому issue просит проверить и закрыть, если гейты
  есть; работы мы им не заказываем.

- [x] Предварительные проверки бюджета §5.2 перед шагом не реализованы @id:budget-precheck-before-step @epic:eco.disputatio
  — **закрыто:** §5.2 переписана как прогноз по медиане стоимости раундов в
  `DECIDING` (спека — PR #131), реализация — ветка `feat/budget-prediction`
  (снимки `budget_snapshot`, все попытки в расходе, `unreported_turns`).
  — SPEC-001 §5.2 требует проверять бюджет и *перед* запуском каждого шага
  («не начинать раунд, который заведомо не влезет»), а runtime зовёт шаг
  напрямую и начисляет бюджет только после него (`runtime/loop.py:309–311`,
  `_run_step` → `charge_step`); в цикле `drive` бюджет сверяет лишь `decide`.
  Замечено локальным ревью PR #129; долг существовал до §5.2a и ею не вызван.
...
- [x] Прогноз бюджета: хвосты после мержа PR #132 @id:budget-prediction-followups @epic:eco.disputatio — все три закрыты (2026-09-25)
  ...
  1. ✅ **решено 2026-09-25** (ветка `fix/legacy-decision-prediction-bypass`):
     при `tracked_from_start` решение без снимка отвергается всегда, в
     прежней сессии сверка идёт с ядром **с прогнозом**, путь `predict=False`
     удалён, SPEC-001 §4.5 обновлён. Исходная находка:
     решение прежней версии без `budget_snapshot`, подложенное автором в
     одиночной сессии и совпадающее с ядром **без прогноза**, принимается и
     глушит `budget_hit: predicted` этого раунда (`runtime/steps.py`
     `_adopt_prior_version`, сверка с `predict=False`). В пайплайне это ловит
  ...
  3. ✅ **решено 2026-09-25** — докстринг называет действующее правило.
     ...
     пересадка FSM.
  **Решения владельца 2026-09-25:** (а) расход шага, прерванного в живой
  фазе, в v1 не начисляется — явная граница, записана в SPEC-001 §4.1 вместе
  с отказом от обещания строгого ограничения фактического расхода;
  (б) обновлять снимок P9 при каждой записи бюджета **не разрешено** — нужен
  отдельный дизайн, при котором обновление доверенного снимка не узаконивает
  заодно изменения автора; падение e2e показало конфликт, но модель учёта не
  определяет; (в) обход прогноза через `decision.json` старого формата
  (п. 1) остаётся открытым и **разбирается первым**: совместимость со старыми
  артефактами не должна позволять автору отключать проверку.
- [x] maestro: автоповтор workstream'а стирает TDD-состояние @id:maestro-retry-wipes-tdd-state @blocked_by:todo://maestro/workstream-retry-wipes-tdd-state
  ...
  `spec-runner tdd repair` работает по живой БД и требует активный чекпоинт
  (CAS по `checkpoint_id`). Значит операторские remedy, которые мы же
  заказывали в D7-A, под maestro недостижимы — оператор приходит к пустому
  состоянию. Отключить повтор из `project.yaml` нельзя: `max_retries` есть
