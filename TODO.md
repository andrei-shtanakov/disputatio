...
- [ ] Автоматизация полировки пары спека+план протоколом раундов @id:spec-pair-polish-automation
  — **PR #51 ЗАБЛОКИРОВАН к merge** (решение владельца 2026-08-30): терминальное
  ревью нашло **13 находок (12 major + 1 minor)**, из них пять — fail-open в защитах, ради
  которых репо существует. Кит `review-pr.sh` отказал на цельной ветке по
  размеру (104 файла / 1.19 МБ против потолков 30 / 400 КБ; chunked-режим не
  реализован, `@id:review-kit-large-pr-mode`), поэтому ветка прогнана
  ...
  **не публиковался**: штатный путь отказал, отпечаток не вычислен, ручное
  ...
  Ветка не запушена — решение о пуше и фикс-PR за владельцем. Сверх находок под
  ...
  `events/{export,atomic}.py`, `runtime/pipeline_export.py`. Повторное
  ревью — штатным `review-pr.sh` на фикс-PR (влезает в потолки); вендорённый
  кит не править (пин `steward @ e4c43cc`), нужен chunked — issue в `steward`
  по ADR-ECO-006.
