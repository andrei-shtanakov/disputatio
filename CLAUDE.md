...
`DECIDING` checks stopping conditions **strictly top-down, first match is terminal** (§5): converged → budget hit → no evidence (§5.2a: `overall == indeterminate` outside the `analyze`-with-empty-gate-set carve-out, reason `verification_indeterminate`) → oscillation → max_rounds; the budget *prediction* (§5.2, `budget_hit: predicted`, per-resource median of round costs derived from `decision.json` `budget_snapshot`) runs last, only when the decision would otherwise be `CONTINUE`. Two rules that are easy to get wrong:
...
- **Исключение — `maestro orchestrate`, и оно требует пост-шага.** При
  ...
  скажет:** пока PR не смержен, локальный master просто впереди origin, и
  `git pull --ff-only` отвечает «Already up to date» с кодом 0 (проверено).
  Громко он откажет только после мержа PR, когда истории разойдутся, — то
  есть позже и не там, где ошибка возникла. Единственный надёжный детектор —
  сравнение SHA. Обоснование двухрежимного правила (волна →
  интеграционная ветка, одиночный прогон → master) — в шапке `project.yaml`.
