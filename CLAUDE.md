...
- **Исключение — `maestro orchestrate`, и оно требует пост-шага.** При
  ...
  После каждого прогона: `git rev-parse master origin/master` — и если
  разошлись, `git reset --hard origin/master`, работа остаётся в ветке
  `ws/<id>` и приезжает своим PR. Забыть не страшно, но неприятно: следующий
  `git pull --ff-only` откажет. Обоснование двухрежимного правила (волна →
