...
`indeterminate` — не провал и не успех, а **отсутствие свидетельства**. Наружу он ведёт себя как `fail`: переход в `REVIEWING` не блокирует (ревьюер видит отчёт и взвешивает его сам), `CONVERGED` блокирует (§5.1). Отличие от `fail` — не в поведении контура, а в том, что артефакт и UI не лгут о причине: «проверка провалилась» и «проверок не было» — разные факты, и оператор, глядящий на `verification.json`, обязан их различать.
