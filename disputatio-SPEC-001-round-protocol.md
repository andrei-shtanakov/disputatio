...
Отсюда следствие, которое v1 признаёт открытым: раунд с `indeterminate` вне кармана §5.1 п.2 не сходится ни при каком поведении агентов, но **сам по себе сессию не останавливает** — она доходит до `max_rounds` и объявляет `DEADLOCK` с причиной, называющей симптом («лимит раундов»), а не причину («не выполнено ни одного гейта»). Статически различимые случаи — конфигурация без гейтов вне `analyze` и конфигурация, где все гейты выключены, — отвергаются на старте запуска, до первого вызова агента. Случай «гейт объявлен включённым, но не запустился» статически неразличим и этой защитой не покрыт: отдельное условие остановки со своим кодом причины — предмет решения, а не умолчания.
