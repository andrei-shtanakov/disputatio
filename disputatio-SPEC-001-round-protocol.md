...
- **стоимость раунда N** = снимок раунда N минус снимок раунда N−1 (`budget_snapshot` из `decision.json`). Для раунда 1 вычитаемое — начальный расход сессии, нулевой по построению (§4.1). Для текущего раунда, чьё решение сейчас принимается, «снимок раунда N» — текущий `budget_used`;
...
Совместимость: артефакты прежней версии (`decision.json` без `budget_snapshot`, `session.json` без `unreported_turns`) читаются; недостающий счётчик читается как `0`. Это безопасно: наблюдение требует двух снимков, а снимок пишет только версия, которая ведёт счётчик, поэтому прежние данные в наблюдения не попадают.
