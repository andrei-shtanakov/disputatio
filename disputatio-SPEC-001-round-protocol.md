...
Отсюда следствие, которое v1 признаёт открытым: раунд с `indeterminate` вне кармана §5.1 п.2 не сходится ни при каком поведении агентов, но **сам по себе сессию не останавливает**. Она доходит до `max_rounds` и уходит штатным путём `DEADLOCK → ESCALATED → EXPORTING(partial)` — то есть отдаёт частичный результат с честным `converged: false`, — но причина остановки называет симптом («лимит раундов»), а не причину («не выполнено ни одного гейта»), и `overall` в манифест не выводится вовсе.

Запретом на старте это не лечится, хотя часть случаев (конфигурация без гейтов вне `analyze`; все гейты выключены; гейт с невыполнимой командой) статически различима: отказ отнял бы у сессии тот самый частичный экспорт, который §2 обещает всегда, а `develop` без детерминированных гейтов — законный сценарий, а не обязательно ошибка конфигурации. Правильное лечение — отдельное условие остановки §5 со своим кодом причины и своим местом в строгом порядке проверок: тогда сессия останавливается на первом же раунде без свидетельств, а не на исходе бюджета, и причина названа честно. Это предмет решения, а не умолчания.

Значение добавлено в `disputatio/v1` расширением, и версия схемы не поднимается: артефакты прежних сессий читаются без изменений. Обещание одностороннее и названо здесь именно поэтому — **forward-совместимость v1 не обещана**: читатель, чья копия схемы знает два значения, отвергнет новый артефакт с тем же тегом `disputatio/v1`. Это принято сознательно, потому что все читатели `verification.json` — эта же кодовая база: артефакт перечитывает resume (`runtime/history.load_verification`), а наружу уходит `result/` с `manifest.json`, где значения `overall` нет вовсе. Внешний потребитель, если появится, пинится версией кода, а не тегом схемы; правило «несовместимое изменение поднимает до v2» относится к смыслу и структуре полей, а не к расширению множества значений закрытого перечисления.
