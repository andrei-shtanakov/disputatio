...
**Агрегация `overall` (нормативно).** Значение выводится из `gates` по трём правилам, проверяемым сверху вниз:

1. есть хотя бы один `fail` ⇒ `fail`;
2. иначе есть хотя бы один `pass` ⇒ `pass`;
3. иначе (набор пуст либо состоит из одних `skip`) ⇒ `indeterminate`.

`pass` требует **хотя бы одной фактически выполненной проверки**. `skip` по-прежнему не провал — отключённый или не запустившийся гейт ничего не опровергает, — но и доказательством он не является: набор, в котором не выполнено ни одного гейта, не доказывает ничего, и выдавать за него зелёный вердикт значит держать fail-open в самом приборе.

`indeterminate` — не провал и не успех, а **отсутствие свидетельства**. Наружу он ведёт себя как `fail` в двух правилах из трёх: переход в `REVIEWING` не блокирует (ревьюер видит отчёт и взвешивает его сам), `CONVERGED` блокирует (§5.1). Отличие от `fail` — не только в честности артефакта («проверка провалилась» и «проверок не было» — разные факты, и оператор, глядящий на `verification.json`, обязан их различать), но и в третьем правиле, где эквивалентности **нет**.

Третье правило — валидация ревью §4.4: `approve` при `overall == fail` отвергается, при `overall == indeterminate` **не отвергается**. Распространить запрет на `indeterminate` значило бы закрыть карман §5.1 п.2: законное ревью `analyze`-сессии с пустым набором уходило бы в schema-retry и после исчерпания `schema_retries` роняло сессию в `FAILED` вместо `CONVERGED`. Сходимость при `indeterminate` в прочих режимах блокирует критерий §5.1, а не валидация ревью — ровно один сторож на одно правило.

Единственный законный путь к сходимости без выполненных гейтов — карман §5.1 п.2 для режима `analyze` с пустым набором: там отсутствие гейтов — свойство задачи, а не дефект конфигурации. Во всех прочих режимах `indeterminate` означает, что сходиться не на чем.

Значение добавлено в `disputatio/v1` расширением: артефакты прежних сессий читаются без изменений, обратной несовместимости нет, версия схемы не поднимается.
...
2. `verification.overall == pass` (или gates пусты в режиме `analyze` — единственный случай, когда `indeterminate` не препятствует сходимости; §4.3);
