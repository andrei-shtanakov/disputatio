...
Статус: draft v0.1 · Дата: 2026-08-07 (§5.2a — 2026-09-25)
...
```json
{
  ...
  "reason": "machine-readable: approve_with_gates_pass | verification_indeterminate | max_rounds | oscillation | ...",
...
Место в порядке — после `BUDGET_HIT` и до осцилляции: исчерпанный бюджет — более сильная причина остановки, а осцилляция и `max_rounds` при отсутствии свидетельства называли бы симптом. Проверки бюджета *перед* шагом (§5.2) не меняются. `CONVERGED` при `indeterminate` вне кармана невозможен по §5.1 п.2, поэтому с ним условие не конкурирует.
