...
### 5.5 ESCALATED
Для v1 эскалация = пользователю: TUI показывает обе позиции (последний proposal + последний review + verification) и три кнопки: «принять как есть», «принять с issues в отчёт», «прервать». Частичный результат экспортируется всегда, с `manifest.json`, где честно указано `converged: false`, причина остановки `stop_reason` (код `reason` из `decision.json` раунда-источника) и список открытых issues. Манифест также несёт `verification_overall` — значение `overall` из `verification.json` раунда-источника (`pass | fail | indeterminate`), либо `null`, если отчёта верификации у этого раунда нет. Поле печатается при любом исходе, в том числе при `CONVERGED`: оно дополняет причину остановки, а не заменяет её. Arbiter-агент — v2, и его вердикт тоже должен проходить валидацию §4.4.
