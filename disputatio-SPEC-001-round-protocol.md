...
Отсюда следствие, которое v1 признаёт открытым: раунд с `indeterminate` вне кармана §5.1 п.2 не сходится ни при каком поведении агентов, но **сам по себе сессию не останавливает** — она доходит до `max_rounds` и объявляет `DEADLOCK` с причиной, называющей симптом («лимит раундов»), а не причину («не выполнено ни одного гейта»). Статически различимые случаи — конфигурация без гейтов вне `analyze`, конфигурация, где все гейты выключены, и гейт с невыполнимой командой — отвергаются при **заведении новой сессии**, до первого вызова агента. Два случая остаются непокрытыми, и оба намеренно:

- «гейт объявлен включённым, но не запустился» (нет бинаря) статически неразличим — увидеть это можно только по результату раунда;
- унаследованная сессия, продолжаемая по снапшоту конфига, не проверяется: отказ отнял бы у неё частичный экспорт, к которому она ещё способна прийти штатным путём `DEADLOCK → ESCALATED → EXPORTING`, — а он и есть то, ради чего продолжение запускают.

Общее лечение у обоих одно и лежит не в пре-флайте: остановка сессии по первому раунду без единого выполненного гейта — отдельное условие §5 со своим кодом причины, то есть предмет решения, а не умолчания.
