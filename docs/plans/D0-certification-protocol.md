...
| 7. подтвердить зелёный без сброса кэша | `uv run pytest -q tests/test_smoke.py::test_package_importable`, при этом `__pycache__` не трогали ни разу за пробу | exit 0 → OK; exit 1 → **не провал сразу**, а диагностика ниже |
...
| D-2 | `uv run pytest -q tests/test_smoke.py::test_package_importable` | exit 0 ⇒ причина в кэше; exit 1 ⇒ причина в дереве |
