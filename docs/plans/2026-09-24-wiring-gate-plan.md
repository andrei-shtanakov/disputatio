...
- [ ] **Шаг 3:** мутация — отсечение по накопленному множеству вместо
  активного стека → краснеет тест `pkg.self.self.alias.C()` (повторный
  `member(pkg, self)` отсекается как повтор); тест независимости разных
  запросов — отдельная мутация: накопленное множество без удаления ключей
  и без сброса между запросами. Общий, но корректно очищаемый стек —
  эквивалентная мутация, краснеть от неё тест не обязан. Записать обе.
