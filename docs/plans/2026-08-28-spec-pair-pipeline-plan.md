...
- `def validate_doc_review(review: Review, *, contour: Literal["spec","pair"],
  ...
  (пустой = валидно); тот же механизм schema-retry.
- **Порядок относительно деградации задан явно: V5/V7/V8 смотрят исходный
  `review`, ДО `degrade_unevidenced_issues`.** Действующий конвейер
  деградирует безевиденсный blocker/major до `minor` первым шагом
  (`validation.py:74`, `:109`) и отдаёт наружу именно деградированную
  копию (`steps.py:504`). Получив её, doc-валидация blocker'а уже не
  увидела бы: `approve` с безевиденсным blocker'ом, `S1: pass` и без
  `defect_class` прошёл бы все три правила, и pair-сессия объявилась бы
  converged вместо schema-retry. Поэтому `validate_doc_review`
  встраивается в конвейер §4.4 **перед** деградацией, а не поверх
  результата; деградация продолжает работать как раньше для остальных
  проверок.
...
- [ ] Перекрёстный тест порядка `test_doc_rules_see_severity_before_degrade`:
  `approve` + blocker без evidence + `S1: pass` + отсутствующий
  `defect_class` — V5/V7/V8 срабатывают (schema-retry), несмотря на то что
  существующая деградация понизила бы этот blocker до `minor`.
...
- `result/`: `pr_title.txt`, `pr_body.md` (сводка чеклистов/evidence,
  ...
  *(Уточнено после ревью PR #51, находка C1: marker снимается ПЕРВЫМ, а не
  переживает повтор как «файл внутри нового набора». Действующее правило —
  §8.2 SPEC-002: уже экспортированный отдельный файл остаётся целым; при
  прерванном повторном экспорте commit marker удаляется; набор считается
  невалидным до успешного нового marker.)*
...
| §10 тесты | 15 (11 core write-ahead границ), 16 (6 операторских границ + анкер), 17 (7 сквозных) + red-тесты задач 1–14 | перечислены поимённо, а не счётом |
